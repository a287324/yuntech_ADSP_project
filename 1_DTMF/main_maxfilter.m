...
windowSize = 10;     % window size of LPF
T = 0.08;               % threshold
...
sMax = ordfilt2(sAbs, windowSize, true(windowSize, 1)); % y(n) = max(x(n), x(n-1), ...)
figure(fpTime);     subplot(4,1,3); plot(sMax);    title('Max(abs(音訊))');
