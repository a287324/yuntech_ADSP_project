...
% FNgroup = ceil(Lx/8);   % number of group of frames
...
for k=1:FN
    ...
    % 時域移位
    if (mod(k, Ngroup) == 0) && (k < FN)
        xout(((k-8)*256+1):(k*256)) = flipud(xout(((k-8)*256+1):(k*256)));
    elseif (k == FN)
        xout(((k-8)*256+1):end) = flipud(xout(((k-8)*256+1):end));
    end
end
